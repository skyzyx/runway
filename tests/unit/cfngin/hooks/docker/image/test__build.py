...
def test_build(cfngin_context: MockCfnginContext, mocker: MockerFixture, tmp_path: Path) -> None:
    ...
    mocker.patch.object(ImageBuildArgs, "model_validate", return_value=args)
    ...
    assert build(context=cfngin_context, **args.model_dump()) == docker_hook_data
...
class TestImageBuildArgs:
    ...
    def test__set_ecr_repo_from_dict(self, tmp_path: Path) -> None:
        ...
        obj = ImageBuildArgs.model_validate({"path": tmp_path, "ecr_repo": args})
        assert obj.ecr_repo
