...
class TestRunwayDeploymentDefinition:
    ...
    def test_modules_setter(self) -> None:
        ...
        assert obj._data.modules[0].model_dump() == new_modules[0].data
        assert obj._data.modules[1].model_dump() == new_modules[1].data
    ...
    def test_parse_obj(self) -> None:
        ...
        assert obj._data.model_dump(exclude_unset=True) == data
    ...
    def test_parse_obj_list(self) -> None:
        ...
        assert isinstance(result, list)
        assert len(result) == 1
        assert result[0]._data.model_dump(exclude_unset=True) == data[0]
    ...
    def test_reverse(self) -> None:
        """Test reverse."""
        data: RunwayDeploymentDefinitionModel = RunwayDeploymentDefinitionModel.model_validate(
            {
                "name": "test",
                "modules": [
                    {"name": "test-01", "path": "./"},
