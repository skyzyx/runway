...
def mock_format_results(mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch.object(EcrLookup, "format_results", return_value="EcrLookup.format_results()")


class TestEcrLookup:
    """Test runway.lookups.handlers.ecr.EcrLookup."""

    def test_get_login_password(self, cfngin_context: MockCfnginContext, runway_context: MockRunwayContext) -> None:
        ...
        with cfngin_stubber, runway_stubber:
            assert EcrLookup.get_login_password(cfngin_context.get_session().client("ecr")) == password
            assert EcrLookup.get_login_password(runway_context.get_session().client("ecr")) == password
    ...
    def test_get_login_password_raise_value_error(self, runway_context: MockRunwayContext) -> None:
        ...
        with (
            runway_stubber,
            pytest.raises(ValueError, match="get_authorization_token did not return authorizationData"),
        ):
            assert EcrLookup.get_login_password(runway_context.get_session().client("ecr"))
    ...
    def test_handle_login_password(
        ...
        assert EcrLookup.handle("login-password", runway_context) == mock_format_results.return_value
